...
def main():
    ...
    # --delete-ecluded implies --delete, and there is no good reason to
    # allow --no-delete
    if delete_excluded:
        COMMANDLINE.append('--delete-excluded')

    # Let rsync resolve the path of the directory (pass the module
    # parameter value verbatim to rsync commandline)
    if link_dest:
        COMMANDLINE.append("--link-dest=%s" % link_dest)

    # Setup the ssh transport of the rsync protocol. Be sure arguments
    # with white spaces will be passed correctly to both rsync AND ssh.
    ...
    # Setup which remote rsync command to run, and which remote user to
    # run as
    if rsync_path:
    ...
    # parameters, or to override module parameters with --no-OPTION
    # options.
    if rsync_opts:
        COMMANDLINE.extend(rsync_opts)

    # These rsync options (--dry-run and --out-format) MUST come at the
    # very end to not be overridden by rsync_opts contents
    # (--no-dry-run, another format for --out-format).
    if module.check_mode:
        COMMANDLINE.append('--dry-run')

    # All the stuff with --out-format, the marker and its further
    # cleanup, diff mode and so on came first from the 'synchronize'
    # module.
    marker = '<<CHANGED>>'
